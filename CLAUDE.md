...
**Decision made (2026-07-31): go positive-only** — `preference(f) = worn(f) / (worn(f) + 5)`, no `shown(f)` term. Rationale: it needs no schema change, preserves the "a suggestion is never persisted unless/until it's worn" principle above, and with a two-person user base the weak "shown but skipped" negative signal would add noise, not accuracy. It's the reversible choice — the negative-signal version (which requires persisting every generated suggestion) can layer on later if positive-only proves too weak. Not yet implemented; this is the agreed shape when it is.
